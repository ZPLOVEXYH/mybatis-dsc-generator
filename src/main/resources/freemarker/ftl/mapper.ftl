...
<mapper namespace="${daoUrl}.${entityName}Mapper">
	...
	<#list cis as ci>
		<#if ci.column!="id">
		<result column="${ci.column}" jdbcType="${ci.jdbcType?upper_case}" property="${ci.property}" />	<!-- ${ci.comment} -->
		<#else>
		<id column="${ci.column}" jdbcType="${ci.jdbcType?upper_case}" property="${ci.property}" />		<!-- ${ci.comment} -->
		</#if>
	</#list>
	</resultMap>
